  ...
  return Promise.all(localFiles.map(file => {
    ...
    getFilesFromCOS(cos, bucketConfig, args.pathPrefix)
      ...
      .then(remoteFiles => {
        ...
        let diffRemote = remoteFiles.filter(x => !local.has(x.Key))
        if (args.deleteInPathPrefix && diffRemote && diffRemote.length > 0) {
          log.info('Deleting files from COS...')
          deleteFileFromCOS(cos, bucketConfig, diffRemote.map(value => { return { Key: value.Key } }))
            .catch(err => {
              console.log(err)
            })
            .then(data => {
